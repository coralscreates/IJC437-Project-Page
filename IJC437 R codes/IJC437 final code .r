...
#Step 1: import the dataset_songs.csv file into R studio 
dataset_songs <- read.csv("dataset_songs.csv") #this reads the csv file

#2: Exploratory Data Analysis(EDA): Here I am conducting an EDA of data for acoustic features individually (there are 9 features)
...
#3. A range of different packages used for EDA: tidyverse, MASS,rgl, corrplot (if package not install use this function install.packages("[insert package name]"))
library(tidyverse)
library(MASS)
library(rgl)
...
library(corrplot)
#4: scatterplots for popularity and each of the nine acoustic features were conducted
...
#5. Now creating a correlation matrix to view all correlations in my graph
...
#6. Creating a histogram to look at the individual distribution of each variable (popularity and each acoustic feature)
...
#9. Now to prepare data for Multiple Linear Regression analysis the data has to be split into train and test
...
#10. now lets move onto prediction of linear regression model using train dataset
...
#11. Here we are calulcating different things, Sum of Squared Errors (SSE), Mean Squared Error (MSE) and the Root Mean Square Error(RMSE)
...
#12. Time for the EDA and visualise the predicted vs actual values
library(ggplot2)
...
# 13: Now moving onto the second regression which looks at the acoustic features that research has highlighted as most signifincant 
...
#14: Now onto the looking at the test data set
...
#15. Now moving onto Question 2: does song type (collaboration vs solo) predict popularity?

#16. Running the EDA for this question to look at what data looks like
str(dataset_songs$popularity.x)
...
#17. Carrying out the regression model just as in previous steps
...
#18. now running regression on the test dataset
...
#19. Now moving onto question 3: Does artist type influence song popualirty 
...
#20. Now the final regression model for question 3
...
#21. now running regression on the test dataset
...
#22. Running a Random Forest for artist type and popualrity 
